import cssData from './gz-nav.css';
import { GzDataElement } from '../GzDataElement';

window.customElements.define('gz-nav', class extends GzDataElement {
    ...
    render(){
      ...
      if(!slotName) 
        if(this.dataSet) slotName = this.dataSet; 
        else slotName = "default";

      //console.log('gz-nav dataSet', this.dataSet);
